...
## Definir Local
setwd("C:/Users/mario.valente/Documents/GitHub_scripts/TCC_DIEGO")
...
Base_Completa=read.table("microdados_enade_2017_arq3.txt", 
                         header=TRUE, 
                         sep=";", 
                         dec=".", 
                         colClasses=c(NT_OBJ_FG="numeric"))
...
### Filtrar as Variáveis SocioEconÃ´micas ########################################################
...
### SocioDemografica ###########################################################################
# CO_REGIA_CURSO     : CÃ³digo da Regiao de Funcionamento do Curso;
# QE_I01             : CÃ³digo do Estado Civil dos Alunos;
# QE_I02             : CÃ³digo da RaÃ§a dos Alunos;
# CO_TURNO_GRADUACAO : CÃ³digo do Turno de Matricula dos Alunos;
...
# Filtrar somente as linhas(alunos) da InstituiÃ§Ã£o (UFPA).
Base_Completa = subset(Base_Completa, Base_Completae$CO_IES == 792);
################################################################################################

### Selecionar um Municipio: Belém #############################################################
# Filtrar somente as linhas(alunos) do Municipio de BelÃ©m 
...
#Filtrando os Dados p/Ciencia da Computação
...
DIREITO = Base_cesupa %>% mutate(RENDA = case_when(QE_I08 == "A" ~ "Até 1,5 S.M",
...
grafico_geom_density1=ggplot(dados,aes(NT_OBJ_FG,fill=RACA))+
  ...
  xlab("Nota")+
  ylab("Densidade")+
  ggtitle("GrÃ¡fico de Densidade das Notas dos Alunos por RaÃ§as e RegiÃµes")+
  facet_grid(~REGIAO)
...
dados = DIREITO_SEM_NA
grafico_histograma1 = ggplot(dados, aes(x=NT_OBJ_FG,fill=RACA))+ 
  geom_histogram() +
...
cronbach.alpha(DIREITO)


### Análise-de-Fatores/Pacote: psych ############################################################
cortest.bartlett(DIREITO, n = NULL, diag=TRUE) 
KMO(DIREITO)
irt.fa(DIREITO_SEM_NA)
fa.parallel(DIREITO_SEM_NA)
#################################################################################################
